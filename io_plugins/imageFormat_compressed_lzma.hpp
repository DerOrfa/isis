...
extern const uint32_t no_compression;
extern const uint32_t best_speed;
extern const uint32_t best_compression;
extern const uint32_t default_compression;

// Status codes

extern const int okay;
extern const int stream_end;
extern const int unsupported_check;
extern const int mem_error;
extern const int options_error;
extern const int data_error;
extern const int buf_error;
extern const int prog_error;

// Flush codes

extern const lzma_action finish;
extern const lzma_action full_flush;
extern const lzma_action sync_flush;
extern const lzma_action run;
...
struct lzma_params {
	...
	lzma_params( uint32_t level = lzma::default_compression ) : level( level )  { }
	uint32_t level;
};
...
//
class lzma_error : public BOOST_IOSTREAMS_FAILURE
{
public:
...
template<typename Alloc> struct lzma_allocator_traits {
...
template < typename Alloc, typename Base =  /*VC6 workaround (C2516)*/ BOOST_DEDUCED_TYPENAME lzma_allocator_traits<Alloc>::type >
...
//
template<typename Alloc = std::allocator<char> > class lzma_decompressor_impl : public lzma_base, public lzma_allocator<Alloc>
{
public:
...
//
template<typename Alloc = std::allocator<char> > struct basic_lzma_compressor: symmetric_filter<detail::lzma_compressor_impl<Alloc>, Alloc> {
private:
...
public:
	...
	basic_lzma_compressor( const lzma_params& = lzma::default_compression, int buffer_size = default_device_buffer_size );
...
//
template<typename Alloc = std::allocator<char> >
struct basic_lzma_decompressor : symmetric_filter<detail::lzma_decompressor_impl<Alloc>, Alloc> {
private:
...
void *lzma_allocator<Alloc, Base>::allocate( void *self, size_t items, size_t size )
{
...
template<typename Alloc, typename Base> void lzma_allocator<Alloc, Base>::deallocate( void *self, void *address )
{
...
template<typename Alloc> lzma_compressor_impl<Alloc>::lzma_compressor_impl( const lzma_params &p )
{ init( p, true, static_cast<lzma_allocator<Alloc>&>( *this ) ); }

template<typename Alloc> lzma_compressor_impl<Alloc>::~lzma_compressor_impl()
{ /* reset(true, false); */ }

template<typename Alloc> bool lzma_compressor_impl<Alloc>::filter( const char*& src_begin, const char *src_end, char*& dest_begin, char *dest_end, bool flush )
{
...
template<typename Alloc> void lzma_compressor_impl<Alloc>::close() { reset( true, false ); }

//------------------Implementation of lzma_decompressor_impl------------------//

template<typename Alloc> lzma_decompressor_impl<Alloc>::lzma_decompressor_impl( const lzma_params &p )
{ init( p, false, static_cast<lzma_allocator<Alloc>&>( *this ) ); }

template<typename Alloc> lzma_decompressor_impl<Alloc>::~lzma_decompressor_impl()
{ /* reset(false, false); */ }
...
template<typename Alloc> bool lzma_decompressor_impl<Alloc>::filter( const char*& src_begin, const char *src_end, char*& dest_begin, char *dest_end, bool /* flush */ )
{
	before( src_begin, src_end, dest_begin, dest_end );
...
template<typename Alloc> void lzma_decompressor_impl<Alloc>::close() { reset( false, false ); }

} // End namespace detail.

//------------------Implementation of lzma_compressor-----------------------//

template<typename Alloc> basic_lzma_compressor<Alloc>::basic_lzma_compressor( const lzma_params &p, int buffer_size ) : base_type( buffer_size, p ) { }

//------------------Implementation of lzma_decompressor-----------------------//

template<typename Alloc> basic_lzma_decompressor<Alloc>::basic_lzma_decompressor( int buffer_size ): base_type( buffer_size ) { }

template<typename Alloc> basic_lzma_decompressor<Alloc>::basic_lzma_decompressor( const lzma_params &p, int buffer_size ) : base_type( buffer_size, p ) { }
