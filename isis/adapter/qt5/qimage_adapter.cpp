...
QImage isis::qt5::makeQImage(const std::vector<data::ValueArray> &lines, data::scaling_pair scaling)
{
	...
	else LOG(Runtime, error)
		<< "Sorry Images larger than 32768x32768 pixels are not supportet, returnung an empty QImage";
	return ret;
}
...
QImage isis::qt5::makeQImage(const std::vector<data::ValueArray> &lines,
							 const std::function<void(uchar *, const data::ValueArray &)> &transfer_function)
{
	QImage ret;
	size_t line_length = lines.front().getLength();
	if(lines.size() <= 32768 && line_length <= 32768){
		...
		<< "Sorry Images larger than 32768x32768 pixels are not supported, returning an empty QImage";
	return ret;
}
QImage isis::qt5::makeQImage(const data::ValueArray &slice,
							 size_t line_length,
							 const std::function<void(uchar *, const data::ValueArray &)> &transfer_function)
{
...
{
	LOG_IF(data.getLength() % line_length, Debug, error) << "The length of the array (" << data.getLength()
														 << ") is not a multiple of the given line length for the QImage, something is really wrong here";
	fillQImage(dst, data.splice(line_length), scaling);
}

void isis::qt5::fillQImage(QImage &dst,
						   const std::vector<data::ValueArray> &lines,
						   const std::function<void(uchar *, const data::ValueArray &)> &transfer_function)
{
...
void isis::qt5::fillQImage(QImage &dst,
						   const data::ValueArray &data,
						   size_t line_length,
						   const std::function<void(uchar *, const data::ValueArray &)> &transfer_function)
{
	LOG_IF(data.getLength() % line_length, Debug, error)
		<< "The length of the array (" << data.getLength()
		<< ") is not a multiple of the given line length for the QImage, something is really wrong here";
