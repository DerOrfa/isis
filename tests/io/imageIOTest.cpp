...
BOOST_AUTO_TEST_CASE ( imageNameGenTest )
{
	...
	BOOST_CHECK_EQUAL( io::FileFormat::makeFilename( img, "/tmp/S{acquisitionNumber}.nii" ), "/tmp/S0.nii" );
	BOOST_CHECK_EQUAL( io::FileFormat::makeFilename( img, "/tmp/S{nich da}.nii" ), "/tmp/S{nich da}.nii" ); // {nich da} does not exist - so we do not touch it
	BOOST_CHECK_EQUAL( io::FileFormat::makeFilename( img, "/tmp/S{acquisitionNumber}_acq{acquisitionTime}.nii" ), "/tmp/S0_acq1234.nii");
}
...
BOOST_AUTO_TEST_CASE ( imageNameUseFormatTest )
{
	...
	// no format string
	BOOST_CHECK_EQUAL( io::FileFormat::makeFilename( props, "/tmp/S{acquisitionNumber}.nii" ), "/tmp/S0.nii" );
	// minimum 0 with format
	BOOST_CHECK_EQUAL( io::FileFormat::makeFilename( props, "/tmp/S{acquisitionNumber%d}.nii" ), "/tmp/S0.nii" );
	// some value  with format
	props.setValueAs<uint32_t>( "acquisitionNumber", 123 );
	BOOST_CHECK_EQUAL( io::FileFormat::makeFilename( props, "/tmp/S{acquisitionNumber%05d}.nii" ), "/tmp/S00123.nii" );
	// maximum with format
	props.setValueAs<uint32_t>( "acquisitionNumber", 4294967295 );
	BOOST_CHECK_EQUAL( io::FileFormat::makeFilename( props, "/tmp/S{acquisitionNumber%05u}.nii" ), "/tmp/S4294967295.nii" );

	BOOST_CHECK_EQUAL( io::FileFormat::makeFilename( props, "/tmp/S{acquisitionTime%.4f}.nii" ), "/tmp/S3.1416.nii" );
	

	// multiple
	BOOST_CHECK_EQUAL( io::FileFormat::makeFilename( props, "/tmp/{acquisitionNumber%u}_{acquisitionTime%.4f}.nii" ), "/tmp/4294967295_3.1416.nii" ) ;
}
